...
/** Adds the missing classic crab burrows to already generated maze chunks. */
@Mod.EventBusSubscriber(modid = ThaumcraftModern.MOD_ID)
public final class OuterLandsCrabVentMigrationEvents {
    private OuterLandsCrabVentMigrationEvents() {
    }
    ...
    static int repairLoadedChunk(ServerLevel level, LevelChunk chunk) {
        ...
        int placed = OuterLandsCrabVents.populate(
                level, chunk.getPos(), level.getSeed(), located.cell()
        );
        if (placed > 0) {
